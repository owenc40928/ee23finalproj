...
def find_fundamental(freqs: np.ndarray, magnitudes: np.ndarray,
    ...
    #  restrict search to piano range
...
def freq_to_piano_key(freq: float) -> dict:
    ...
    # ecnts deviation: 100 cents = 1 semitone
    ...
    try:
        ...
        for i, d in enumerate(devices):
            if d["max_input_channels"] > 0:
                ...
                print(f"  [{i:2d}] {d['name']}  "f"(channels: {d['max_input_channels']}, "f"rate: {int(d['default_samplerate'])} Hz){marker}")
        print()
    except ImportError:
...
def record_audio(duration: float = 2.0, sample_rate: int = 44100, device=None) -> np.ndarray:
...
def plot_spectrum(freqs: np.ndarray, magnitudes: np.ndarray, detected_freq: float, key: dict, title: str = ""):
    ...
    mask = freqs <= 5000
    ax.plot(freqs[mask], magnitudes[mask], color="#2196F3", linewidth=0.8, label="FFT Magnitude")

    if detected_freq > 0:
        ax.axvline(x=detected_freq, color="#FF5722", linewidth=1.5, linestyle="--", label=f"Detected: {detected_freq:.1f} Hz")
        ax.axvline(x=key["freq"], color="#4CAF50", linewidth=1.5, linestyle=":", label=f"Matched: {key['name']} ({key['freq']:.2f} Hz)")
...
def analyze_signal(signal: np.ndarray, sample_rate: int, show_plot: bool = True) -> dict:
    ...
    # 1. Filter
    filtered = bandpass_filter(signal, lowcut=27.0, highcut=4200.0, sample_rate=sample_rate)
...
def main():
    ...
    parser.add_argument("--duration", "-d", type=float, default=2.0, help="Recording duration in seconds (default: 2.0).")
    parser.add_argument("--sample-rate", "-r", type=int, default=44100, help="Sample rate in Hz for live recording (default: 44100).")
    ...
    parser.add_argument("--no-plot", action="store_true", help="Disable the FFT spectrum plot.")
    parser.add_argument("--loop", "-l", action="store_true", help="Keep recording and identifying until Ctrl+C.")
